...
local function pendNotif(Title,Desc,Data)
	spawn(function()
		...
		if Data[1] == "Support" then
			local Player = Instance.new('ObjectValue', notifClone)
			Player.Name = "Player"
			Player.Value = Data[2]
		end
...
essentialsEvent.OnClientEvent:connect(function(Starter,...)
	...
	elseif Starter == "Support" then
		local User = Data[1]

		for a,b in pairs(Stacks.Notifs) do
			if b:FindFirstChild('Player') and b:FindFirstChild('Player').Value == User then
				return
			end
		end

		pendNotif("Support Requested", "by " .. User.Name, {'Support', User})
	elseif Starter == "removeSupport" then	
		local Container = baseClip:WaitForChild("Container")
		for _, Child in next, Container:GetChildren() do
			if Child.Name == 'Notif Clone' and Child:FindFirstChild('Player') then
				if Data[1] == Child:FindFirstChild('Player').Value then
					for Key, Notification in next, Stacks.Notifs do
						if Notification == Child then
							table.remove(Stacks.Notifs, Key)
						end
					end
					Child:Destroy()
					figureNotifs(Stacks.Notifs, Container)
				end
			end
		end	
