...
local sysTable = {
	...
	localNames = {},
	Changelog = [[[5/10/20]
- Fixed bug where "tp", "bring", and "to" would teleport seats with the player.

[5/8/20]
- Fixed bug where without a prefix, capitalized commands, i.e "SM hi", wouldn't work. (Thanks lyookachu!)
	
[3/1/20]
- Updated clean command

If you have any questions or concerns, feel free to PM TheFurryFish on Roblox.]]
}
...
function Funcs.Display(Args)
	...
	elseif Command == "trellobans" then
		if not checkTrello() then
			essentialsEvent:FireClient(Player,'Hint','Error','Trello is not configured.')
			return
		end
		local Table = {}
...
local function awardCape(Player,cColor,cMat,cDecal,cTrans)
	...
	if b.Character and b.Character:FindFirstChild('Humanoid') and not b.Character:FindFirstChild('BAE Cape') then
		...
		if cDecal then
			...
			capeDecal.Texture = "rbxassetid://"..419363338
		end
...
function Funcs.God(Args)
	local Player = Args[1]
	if Args[2] == "god" or Args[2] == "ungod" then
		local Players = returnPlayers(Player,Args[3],Args[2]) if not Players then return end
		for a,b in next,Players do
			if b and b:IsDescendantOf(playerService) and b.Character and b.Character:FindFirstChild('Humanoid') then
				local playerHum = b.Character:WaitForChild('Humanoid')
				...
				elseif Args[2] == "ungod" then
					playerHum.MaxHealth = 100
...
Market.PromptPurchaseFinished:connect(function(Player, Asset, isPurchased)
	if isPurchased then
		if Asset == sysTable.donorID and sysTable.donorPerks then
			...
			essentialsEvent:FireClient(Player,'Hint',"Success",'Purchased Donor Perks')
			essentialsEvent:FireClient(Player,'Donor')
		end
	end
end)
			...
			if Data[1] == "Notification Transfer" then
				...
				elseif Data[2][1] == "Support" then
					...
					if User then
						...
						wait(0.5)
						Funcs.Teleport({Player, 'to', User.Name})
					end
