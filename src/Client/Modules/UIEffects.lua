--[[
	...
	Documentation:
	UIEffects.module.lua
	---------------
	Author:
	@ lightsort
	---------------
	Adds and manages some UI effects.
...
function UIEffects:CameraZoomIn()
	blur.Size = 0
	spr.target(blur, 0.9, 5, { Size = 15 })
	spr.target(camera, 0.9, 5, { FieldOfView = 90 })
end

function UIEffects:CameraZoomOut()
	blur.Size = 12
	spr.target(blur, 0.9, 5, { Size = 0 })
	spr.target(camera, 0.9, 5, { FieldOfView = 70 })
end
...
function UIEffects:HideUIs()
	...
	local HUD = UIController.Buttons
