...
local IconController = require(Knit.Modules.TopbarManager.IconController)
local Themes = require(Knit.Modules.TopbarManager.Themes)
local Icon = require(Knit.Modules.TopbarManager)
...
function UIController:RegisterButtonClick(Button)
    Button.MouseButton1Click:Connect(function()
		...
		if self.UI[Button.Parent.Name].Visible then
			spr.target(self.UI[Button.Parent.Name], 1, 4, { GroupTransparency = 1, Position = UDim2.fromScale(0.5, 0.55)})
			UIEffects:CameraZoomOut()
			CurrentFrame = nil
			
			task.wait(0.15)
			self.UI[Button.Parent.Name].Visible = false
		else
			CurrentFrame = Button.Name

			for _, MenuPages in pairs(self.UI:GetChildren()) do
			...
			self.UI[Button.Parent.Name].Visible = true
			spr.target(self.UI[Button.Parent.Name], 1, 4, { GroupTransparency = 0, Position = UDim2.fromScale(0.5, 0.5)})
...
function UIController:NavigationMenu()
    for Index, MenuButtons in pairs(self.Buttons:GetDescendants()) do
...
function UIController:KnitInit()
	...
	self.Buttons = playerGui:WaitForChild("GochiUI"):WaitForChild("Navigation")
...
function UIController:TopbarMenu()
	RankService:GetRank(Player):andThen(function(Rank: number)
		warn(Rank)
		if Rank >= 60 then
			local icon = Icon.new()
			...
			icon.selected:Connect(function()
				...
				self.UI.ChefQueue.Visible = true
				spr.target(self.UI.ChefQueue, 1, 4, { GroupTransparency = 0, Position = UDim2.fromScale(0.5, 0.5)})
				if not FrameFound then UIEffects:CameraZoomIn() end
				CurrentFrame = "ChefQueue"
			end)

			icon.deselected:Connect(function()
				spr.target(self.UI.ChefQueue, 1, 4, { GroupTransparency = 1, Position = UDim2.fromScale(0.5, 0.55)})
				...
				task.wait(0.15)
				self.UI.ChefQueue.Visible = false
			end)
